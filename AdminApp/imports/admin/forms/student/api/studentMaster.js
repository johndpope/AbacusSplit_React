...
Meteor.methods({
	...
	'setCompetitionPaymentStatusManually':function(studId){
		...
		var student = StudentMaster.findOne({"studentId":studId});
		var category='';	
		if(student){
			category = student.category;	
			if(category){
				var practiceTestData = MyPracticeExamMaster.find({'StudentId':studId,"category":category,"examType":"Practice Exam","examStatus":"Completed"},{fields:{"examPaperId":1}}).fetch({});
				if(practiceTestData){
					for(var i=0;i<practiceTestData.length;i++){
						...
						if(getPaperData && getPaperData.paperStatus == "Assigned"){
							MyPracticeExamMaster.remove({'StudentId':studId,"category":category,"examType":"Practice Exam","examStatus":"Completed","examPaperId":getPaperData._id});
						}
					}
