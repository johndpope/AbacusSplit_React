...
Meteor.methods({
	...
	'generateRankCatSubCatWise':function(category,subCategory,competitionId){
		...
		// var studentCount = MyExamMaster.find({'category':category,'subCategory':subCategory,'competitionId':competitionId},{sort:{"totalScore":-1,"examSolvedTime":1}}).count();
		console.log("allCategoryWiseStudent = " , allCategoryWiseStudent);
		if(allCategoryWiseStudent.length>0){
			var studentCount = allCategoryWiseStudent.length;

			if(studentCount){
				cnt = Math.round(studentCount*0.10);
				var totalMarks = allCategoryWiseStudent[0].totalMarks;
				if(totalMarks){
					var minMarks = totalMarks * 0.5;
					var rank = 1;
					// if(rank){
					...
					console.log(allCategoryWiseStudent);
				}
			}
	...
	'allocateRank': function(studentData,minMarks,rank){
		var studCnt = studentData.length;
		if(studCnt){
			var tenPOfstudCnt = 10;
			// var tenPOfstudCnt =  Math.round(studCnt*0.1);
			// var tenPOfstudCnt =  Math.round(studentData.length*0.1);
		}
	// console.log("cnt",cnt);		
		...
		else if(rank>3 && rank<=cnt && studentData.length>0){
		// else if(rank>3 && rank<=tenPOfstudCnt && studentData.length>0){
			// console.log("in else--cnt->",cnt);
			var time = studentData[0].examSolvedTime;
			var score = studentData[0].totalScore;
			if(score>=minMarks){
				var rankedStudents = studentData.filter(function(data){
