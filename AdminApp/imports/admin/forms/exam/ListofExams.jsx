...
class ListOfExams extends TrackerReact(Component)  {
	...
	competitionView(event){
		var examId = $(event.target).attr('name');
		Meteor.call('competitionView',examId,(error,result)=>{
			if(error){
			
			}else{
				var result1 = result;
				console.log("result",result1);
				if(result1=="changeShowViewStatus"){
										    ...
										    <tbody>
										    	{this.props.showexams.map((allExam, index)=>{
										    		return <tr key={index}>
										    			...
										    			{allExam.competitionView=="Show" ?
										    				<label className="switch" title="Click to stop this competition">
										    					<input type="checkbox" id="togBtn" name={allExam._id} onClick={this.competitionView.bind(this)} checked="checked" />
										    					<div className="slider round">
										    						<span className="on">Show</span><span className="off">Hide</span>
										    					</div>
										    				</label>
										    				:
										    				<label className="switch" title="Click to start this competition">
										    					<input type="checkbox" id="togBtn" name={allExam._id} onClick={this.competitionView.bind(this)} checked={this.state.competitionView==='Hide'} />
										    					<div className="slider round">
										    						<span className="on">Show</span><span className="off">Hide</span>
										    					</div>
										    				</label>
