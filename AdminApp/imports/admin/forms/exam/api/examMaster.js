...
Meteor.methods({
	...
	'allCompetitions':function(){
	  	return ExamMaster.find({"competitionView":"Show"},{fields:{"competitionName":1}}).fetch(); 
	  },
	...
	// Update exam status 
		...
		'competitionView':function(_id){
			var examMasterData = ExamMaster.findOne({"_id":_id});
			if(examMasterData){
				var competitionStatus = examMasterData.competitionView;
				if(competitionStatus == "Show"){
					ExamMaster.update({"_id":_id},
							{
								$set:{
									'competitionView': 'Hide',
