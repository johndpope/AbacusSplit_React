...
export default class PracticeQuestionPaper extends TrackerReact(Component)  {
	...
	componentWillMount(){
  		 Meteor.call("isAuthenticated","MasterData","PracticeQuestionPaperList",(err,res)=>{
			if(err){
				console.log(err);
			}else{
				this.setState({
	...
	showFinalexamPaper(){
		...
		var postHandle = Meteor.subscribe("ShowQuestionPaperCategorywise",this.state.categoryName,this.state.subCategory).ready();
		
		var PracticeQPData =  QuestionPaperMaster.find({"examType":"Practice Exam","category":this.state.categoryName,"subCategory":this.state.subCategory},{sort:{'createdAt':-1}}).fetch();
		if(PracticeQPData){
			return PracticeQPData;
		}else{
			return 0;
		}
	}
	...
	removeQuestionPaper(event){
		...
		swal({
			  ...
			  closeOnConfirm     : false
				...
				Meteor.call("IsQuestionPaperAttempted",_id,(err,res)=>{
					if(err){

							}else if(res){
								console.log("in res",res);
								if(res=='DeletePracticeExam'){
								...
								// 					swal(
								// 					    'You can not delete this Question paper',
								// 					    'This question paper used in competition',
								// 					    'warning'
								// 				  	);
								// 				}else if(result=='qpAssigned'){
								// 					swal(
								// 					    'You can not delete this Question paper',
								// 					    'This question paper assigned for free practice test',
								// 					    'warning'
								// 				  	);
								// 				}else{
								// 					swal(
	...
	updateAssignUnAssignStatus(event){
		var _id = $(event.target).attr('id');
		Meteor.call("AssignOrUnAssignStatus",_id);
	}

	existInPackageOrNot(questionPaperId){
		// console.log("questionPaperId",questionPaperId);
		...
		if(isReady){
			...
			// console.log('packageManagementData ---->',packageManagementData);
	...
	render(){
		if(this.state.facilityPermission != 'waitingforResult' && this.state.facilityPermission == true){
			              ...
			              <div className="col-lg-12 col-md-12 col-xs-12 col-sm-12">
			                <div className="box">
			                  <div className="box-header with-border boxMinHeight">
					            ...
					            <h3 className="box-title">Practice Question Papers</h3>
					            </div>
								<div className="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				                    <div className="col-lg-12  col-md-12 col-sm-12 col-xs-12 selectCatSubCatList">
										...
										<div className="col-lg-4 col-md-4 col-sm-4 col-xs-12">
											<input type="text" name="search" placeholder="Search exam name..." className="col-lg-11 col-md-11 col-sm-11 SearchExam" onInput={this.practTableSearch.bind(this)}/>
										</div>
									</div>
						              		...
						              		<table className="table table-striped formTable" id="ExamListTable">
											    <thead className="tableHeader">
											        <tr>
											            ...
											            <th className="col-lg-5">Exam Title </th>
											            ...
											            <th className="col-lg-1 tab-Table"> Status </th>
											            <th className="col-lg-1 tab-Table"> Action </th>
											            
											        </tr>
											    </thead>
											    {this.showFinalexamPaper() != 0 ? 
											    <tbody className="myAllTable">
											    	{this.showFinalexamPaper().map((questionPaper, index)=>{
											    		return <tr key={index} className={questionPaper.status}>
											    			...
											    			<td><a href={`/Admin/QuestionPapers/${questionPaper._id}`} title="Click to view question paper">{questionPaper.quePaperTitle} &nbsp;&nbsp;&nbsp;<span>{questionPaper.isDraft}</span></a><br/>
											    				<div className="existInPackage">{this.existInPackageOrNot(questionPaper._id)}</div>
											    			</td>
											    			...
											    			<td className="tab-Table">
											    			 {questionPaper.paperStatus =="Assigned" ?
										                       	<button className='PQPBtn btn btn-default' onClick={this.updateAssignUnAssignStatus.bind(this)} id={questionPaper._id} title="Click to UnAssign from Practice Exam">
										                       	{/*<label className="paidStaus">*/}
									                            	{questionPaper.paperStatus}
									                       		{/*</label>*/}
									                       		</button>
									                       	:
									                       		questionPaper.isDraft=='[Draft]' ?
									                       			<button className='PQPBtnNot btn btn-default' title="Please first add questions in question paper" disabled>
									                            	{questionPaper.paperStatus}
									                       		</button>
									                       		:
									                       		<button className='PQPBtnNot btn btn-default' onClick={this.updateAssignUnAssignStatus.bind(this)} id={questionPaper._id} title="Click to Assign this Exam in Practice Exam">
									                            	{questionPaper.paperStatus}
									                       		</button>
											                }
											                </td>
											    ...
											    :
											    	...
											    	{!this.state.subCategory ?
										    			<tr>
										    				<td colSpan="6">"{this.state.categoryName} Category Practice Question Papers not yet available <a href="/Admin/CreateQuestionPapers">click here</a> to add Practice Question Papers"</td>
										    			</tr>
										    			:
										    			<tr>
										    				<td colSpan="6">"{this.state.subCategory} Sub-Category Practice Question Papers not yet available <a href="/Admin/CreateQuestionPapers">click here</a> to add Practice Question Papers"</td>
										    			</tr>
										    		}
										    		</tbody>
									    		}
			...
			}else if (this.state.facilityPermission == false || this.state.facilityPermission == undefined){
