	...
	constructor(){
		...
		this.state ={
			...
			"subscription" : {
				...
				// "firstFranchiseName" : Meteor.subscribe("firstFranchiseName"),
			}
		}
		this.handleChange = this.handleChange.bind(this);
		this.getFranchiseName = this.getFranchiseName.bind(this);
		this.showTestData = this.showTestData.bind(this);
		this.studRegistrationAnnualData = this.studRegistrationAnnualData.bind(this);
	}
	componentDidMount() {
		this.setState({
			categoryName : 'A',
	...
	showTestData(){
	    ...
	    var yearDateEnd = new Date (yearDateStart.getFullYear(), 11, 31);
	  	if(!this.state.studentNameYearly){
	  		...
	  		var studentname=this.state.studentNameYearly;
	  	}
		...
		Meteor.call("allStudentPracticeTestDataMonthly",franchiseLoginId,this.state.categoryName,yearDateStart,yearDateEnd,studentname,(err,res)=>{
	 		this.setState({
			...
			for (var j = 0; j < examArr.length; j++) {
				...
				examArray.push(
					<tr key={j}>
						...
						<td>{examArr[j].questionPaperName}</td>
			...
			{Roles.userIsInRole(Meteor.userId(), ['Admin','superAdmin'])?
					...
					<div className="col-lg-4 col-md-4 col-sm-6 col-xs-6">
						<span className="blocking-span"> 
							<select type="text" name="franchise" ref="franchise" value={this.state.franchise} onClick={this.getFranchiseName.bind(this)} className="form-control subCatTab col-lg-12 col-md-12 col-sm-12 col-xs-12 " onChange={this.handleChange} required>
								...
								{this.state.franchiseData.map((franchise,index)=>{
				...
				<div className="col-lg-3 col-md-3  col-sm-3 col-xs-3 paddingleftzero">
					<div className="col-lg-12 col-md-12 searchboxreport searchlbl1">
				       		<span className="blocking-span">
				           		<input type="text" name="search"  className="col-lg-12 col-sm-4 SearchExam SearchStudentYearlyName inputTextSearch" onInput={this.getYearlyTextValue.bind(this)} required/>
				...
				<div className="col-lg-4 col-md-4  col-sm-4 col-xs-4 paddingleftzero">
					<div className="col-lg-12 col-md-12 bottommargin searchbxtop">
				       		<span className="blocking-span">
				           		<input type="text" name="search"  className="col-lg-12 col-sm-12 SearchExam SearchStudentYearlyName inputTextSearch" onInput={this.getYearlyTextValue.bind(this)} required/>
				           		<span className="floating-label">Search Student Name</span>
				       		</span>
				     </div>
			        </div>
