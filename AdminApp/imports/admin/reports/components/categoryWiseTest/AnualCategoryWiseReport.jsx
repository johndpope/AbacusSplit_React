...
export default class AnualRegistrationReport extends TrackerReact(Component) {
	...
	CategoryAnnualData(){
	    ...
	    // console.log("vikas category ------------> ", category);
	    if(!this.state.CWTYName){
			var examArr = ExamMaster.find({$and:[{'category':this.state.categoryName,'subCategory':this.state.subCategory},{'examDateFormat':{$gte: yearDateStart,$lt: yearDateEnd}}]},{sort:{'examDate':-1}}).fetch();
		}else{
			var examArr = ExamMaster.find({$and:[{$or:[{'category':this.state.categoryName},{'subCategory':this.state.subCategory}]},{'examName':this.state.CWTYName,'examDateFormat':{$gte: yearDateStart,$lt: yearDateEnd}}]},{sort:{'examDate':-1}}).fetch();
		}
		// console.log(examArr);
		var examArray = [];
		if(examArr.length > 0){
	...
	getCWTYTextValue(event){
		var studentName= $('.SearchCWTYName').val();
		if(studentName){
			var RegExpBuildValue = this.buildRegExp(studentName);
			this.setState({
				CWTYName   : RegExpBuildValue,
			});
		}else{
			this.setState({
				CWTYName   : '',
		...
		<div className="col-lg-12 col-md-12">
			...
			<div className="col-lg-12 col-md-12  col-sm-12 col-xs-12 paddingleftzero">
				<div className="col-lg-4 col-lg-offset-8 col-md-12 searchTableBoxAlignSETReport">
		       		<span className="blocking-span">
		           		<input type="text" name="search"  className="col-lg-12 col-sm-12 SearchExam SearchCWTYName inputTextSearch" onInput={this.getCWTYTextValue.bind(this)} required/>
			...
			<form className="todaysSalesReport">
				<div className="">
					<div className="break col-lg-12 col-md-12"></div>
						...
						<table className="table table-striped  table-hover myTable table-bordered todaysSalesReportForpdf reportTables" id="yearlyCategoryWiseTestTaken">
							<thead>
