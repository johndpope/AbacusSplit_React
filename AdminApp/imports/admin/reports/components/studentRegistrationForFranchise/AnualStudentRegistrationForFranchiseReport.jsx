...
export default class AnualStudentRegistrationForFranchiseReport extends TrackerReact(Component) {
	...
	studRegistrationAnnualData(){
	    ...
	    if(!this.state.studentNameYearly){
			var examArr = StudentMaster.find({'franchiseIdForReport':franchiseLoginId,'category':this.state.categoryName,'subCategory':this.state.subCategory,'createdAt':{$gte: yearDateStart,$lt: yearDateEnd}},{sort:{'createdAt':-1}}).fetch();
		}else{
			var examArr = StudentMaster.find({$and:[{$or:[{'franchiseIdForReport':franchiseLoginId,'category':this.state.categoryName},{'subCategory':this.state.subCategory}]},{'studentFullName':this.state.studentNameYearly,'createdAt':{$gte: yearDateStart,$lt: yearDateEnd}}]},{sort:{'createdAt':-1}}).fetch();
		}
		// console.log(examArr);
		var examArray = [];
		if(examArr.length > 0){
