...
export default class UMListOfUsers extends TrackerReact(Component) {
  ...
  componentDidMount(){
    ...
    // console.log("in did---->",FlowRouter.getParam('role'));

    if(FlowRouter.getParam('role')){
        if(this.state.selectedRole == "Franchise"){
          this.setState({
            roleFranchise : 'true',
            selectedRole  : FlowRouter.getParam('role'),
          });
        }else{
          this.setState({
            roleFranchise : 'false',
        ...
        var RegExpBuildValue1 = this.buildRegExp(FlowRouter.getParam('role'));
        this.setState({
      ...
      Session.set("roleSet", FlowRouter.getParam('role'));
      this.usersListData();
      this.paginationUMFunction();
    // var roleSetVar        = Session.get('roleSet');
   //    var activeBlockSetVar = Session.get('activeBlockSet');
  ...
  'roleFilter'(event) {
      ...
      FlowRouter.go('/Admin/UMListOfUsers/'+selectedValue);
      if(selectedValue){
...
/*--------------- Search User --------------*/
  ...
  render(){
    if(this.state.facilityPermission != 'waitingforResult' && this.state.facilityPermission == true){
      ...
      console.log("in render---->",this.state.selectedRole);    
         return(
        <div>
