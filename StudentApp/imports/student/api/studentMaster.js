...
Meteor.methods({
	...
	'getProfilePic':function(id){
		// console.log("id",id);
		if(id){
			var userData = StudentMaster.findOne({"_id":id});
			var TempImgFound = TempImage.findOne({"userId":Meteor.userId()});
			if(TempImgFound){
				return "ImgFound";
			}else{
				var TempImgExist = userData.imgSrc;
				if(TempImgExist){
					return "ImgFound";
				}
			}
	...
	'getUpdateStatus':function(userId){
		// console.log("in api====================>",userId);
		return StudentMaster.findOne({"studentId":userId},{fields:{"profileEditStatus":1}});
	},
	...
	'calculateNetspeed':function(){
		var USERID=Meteor.userId();
		var speedTest = require('speedtest-net');
		var test = speedTest({maxTime: 5000});
			test.on('data', function (data, err) {
			  bound(() => {
			    if (data) {
	...
	'getInternetSpeedParams':function(){
		var studentInfo=StudentMaster.findOne({"studentId":Meteor.userId()},{fields:{"netspeed":1}});
		
		if(studentInfo){
			if(studentInfo.netspeed){
				return studentInfo.netspeed;
			}else{
				return null;
			}
	...
	'credentialExists':function(formValues){
		if(formValues){
			var mailExist   = StudentMaster.findOne({"studentEmail":formValues.studentEmail});
			var mobileExist = StudentMaster.findOne({"mobileNumber":formValues.mobileNumber});
		
		if(mailExist || mobileExist){
			return 'Exists'
		}else{
			return 'Not Exists'
		}
	...
	'setCompetitionPaymentStatus':function(){
		var student = StudentMaster.findOne({"studentId":Meteor.userId()});
		var category='';	
		if(student){
			category = student.category;	
			if(category){
				var practiceTestData = MyPracticeExamMaster.find({'StudentId':Meteor.userId(),"category":category,"examType":"Practice Exam","examStatus":"Completed"},{fields:{"examPaperId":1}}).fetch({});
				if(practiceTestData){
					for(var i=0;i<practiceTestData.length;i++){
						var getPaperData =  QuestionPaperMaster.findOne({"_id":practiceTestData[i].examPaperId},{fields:{"paperStatus":1}});
						if(getPaperData && getPaperData.paperStatus == "Assigned"){
							MyPracticeExamMaster.remove({'StudentId':Meteor.userId(),"category":category,"examType":"Practice Exam","examStatus":"Completed","examPaperId":getPaperData._id});
						}
	...
	'setCompetitionPaymentStatusManually':function(studId){
		...
		// MyPracticeExamMaster.remove({'StudentId':Meteor.userId(),"category":category,"examType":"Practice Exam","examStatus":"Completed"});
	},
	...
	'getAllCategoryWiseStudentCount':function(){
		var studentData =StudentMaster.find({},{sort: {createdAt: -1},fields:{"studentId":1}}).fetch()||{};
		if(studentData){
			return studentData.length;
		}
	},

	'getCategoryWiseStudentCount':function(category,subCategory){
		...
		if(category=="All"){
			if(subCategory=="All"){
				...
				var studentData =StudentMaster.find({"subCategory":subCategory},{sort: {createdAt: -1},fields:{"studentId":1}}).fetch()||{};
				if(studentData){			
					return studentData.length;
				}
			}
			...
			if(subCategory=="All"){
				var studentData =StudentMaster.find({"category":category},{sort: {createdAt: -1},fields:{"studentId":1}}).fetch()||{};
				if(studentData){
					return studentData.length;
				}
			}else{
				var studentData =StudentMaster.find({"category":category,"subCategory":subCategory},{sort: {createdAt: -1},fields:{"studentId":1}}).fetch()||{};
				if(studentData){
					return studentData.length;
				}
	...
	// 'AddStudentProfileUpdate':function(id){
	// 	// console.log("idd---->",id);
	// 	StudentMaster.update({"studentId":id},
	// 	{
	// 		$set:{
