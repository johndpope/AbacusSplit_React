...
if(Meteor.isServer){
	...
	Meteor.publish("showAllCompRegOrderStudentwise",function(id){
		return CompetitionRegisterOrder.find({"studentId":id,"status":"paid"});		
	});
	...
	Meteor.publish("latestCRONewSingle",function(competitionId){
		return CompetitionRegisterOrder.find({"competitionId":competitionId,"studentId":Meteor.userId(),"status":"paid"});		
	});
	...
	Meteor.methods({
		...
		'applyForCompetition':function(compFees,compId,QPId){
			...
			if(studentMasterData && ExamMasterData){
				...
				CompetitionRegisterOrder.insert({
					...
					"competitionOriginalFees"         : compFees,
		...
		'updateOnlineDetailsToOrder':function(studentId,compId,status,transid,billnumbers){
			// console.log("in meteor call of payment",studentId,compId,status,transid,billnumbers);
			var order = CompetitionRegisterOrder.findOne({'competitionId': compId,"studentId":studentId});

			if(order){
			// console.log("in meteor call order",order);
...
//--------------- get competition wise franchise details ----------------------//
	...
	'addCompPayDetails':function(paymentFormValues){
		// console.log("paymentFormValues.competitionId",paymentFormValues.competitionId);
		...
		if(paymentALreadyDone){
			...
			var studenData      = StudentMaster.findOne({"studentId":paymentFormValues.studentId});
			 ...
			 // console.log("competitionData in payment",competitionData);
