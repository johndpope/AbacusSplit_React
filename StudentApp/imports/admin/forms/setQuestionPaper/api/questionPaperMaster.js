...
if(Meteor.isServer){
	...
	Meteor.publish("quesPaperPracticeExam",function(){
		return QuestionPaperMaster.find({"examType":"Practice Exam"});	
	});

	Meteor.publish("quesPaperPracticeExamSingle",function(category,subCategory){
		return QuestionPaperMaster.find({"category":category,"subCategory":subCategory,"examType" : "Practice Exam","isDraft":"","paperStatus" : "Assigned"});	
	});
	...
	Meteor.publish("assignedPracticePaperSingle",function(category,subCategory){
		return QuestionPaperMaster.find({"category":category, "subCategory":subCategory, "examType" : "Practice Exam","isDraft":"","paperStatus" : "Assigned"});	
	});
