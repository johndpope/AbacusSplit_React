...
export default class CompetitionResultView extends TrackerReact(Component) {
		...
		var monthDateFromSess = Session.get("selectedMonth");			
	  	...
	  	var monthDateToSess = new Date(moment(monthDateFromSess).add(1,"M"));
			if(!this.state.studentNameCWTM){
				Meteor.call("getAllResult",this.state.categoryName,this.state.subCategory,this.state.competitionId,this.state.startRange,this.state.dataRange,
					(err,res)=>{
					if(err){
						console.log(err);
					}else{
				...
				Meteor.call("getAllResultSearch",this.state.categoryName,this.state.studentNameCWTM,this.state.competitionId,
					(err,res)=>{
						if(err){
							console.log(err);
						}else{
...
//------- create pagination for table --------------// 
	    ...
	    swal({
		    ...
		    Meteor.call("removeStudentFromSelectedCompetition",studentId,competitionId,(err,res)=>{
		    	if(err){
		    		...
		    		if(res=="studRemoved"){
		    			Meteor.call("getAllResult",categoryName,subCategory,competitionId,startRange,dataRange,
							(err,res)=>{
							if(err){
								console.log(err);
							}else{
	...
	render() {
		// console.log("competitionDeclared in view franchise",this.state.competitionDeclared);
