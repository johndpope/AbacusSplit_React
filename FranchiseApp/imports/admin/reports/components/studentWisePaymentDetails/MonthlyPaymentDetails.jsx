...
export default class MonthlyPaymentDetails extends TrackerReact(Component) {
	...
	studWiseTestMonthlyData(){
	 ...
	 //  		// console.log("Id",ID);
		...
		// 		Meteor.call("getCategoryWiseSWTT",ID,this.state.categoryName,monthDateStart,monthDateToSess,(err,res)=>{
		// 			if(err){
		// 				console.log(err);
		// 			}else{
		// 				if(res){
		...
		// 		Meteor.call("getCategoryWiseSWTTSearch",this.state.categoryName,this.state.subCategory,monthDateStart,monthDateToSess,this.state.studentNameCWTM,(err,res)=>{
		// 			if(err){
		// 				console.log(err);
		// 			}else{
		// 				if(res){
		...
		// 		if(!this.state.studentNameCWTM){
		// 		// var allCategoryWiseStudent = MyExamMaster.find({'category':this.state.categoryName,'subCategory':this.state.subCategory,'examDateFormat':{$gte: monthDateStart,$lt: monthDateToSess}},{sort:{"totalScore":-1,"examSolvedTime":1}}).fetch();
		// 		Meteor.call("getCategoryWiseSWTTFranchise",ID,this.state.categoryName,monthDateStart,monthDateToSess,(err,res)=>{
		// 			if(err){
		// 				console.log(err);
		// 			}else{
		// 				if(res){
		...
		// 	}else{
		// 		// var allCategoryWiseStudent = MyExamMaster.find({$and:[{$or:[{'category':this.state.categoryName},{'subCategory':this.state.subCategory}]},{'fullName':this.state.studentNameCWTM,'examDateFormat':{$gte: monthDateStart,$lt: monthDateToSess}}]},{sort:{"totalScore":-1,"examSolvedTime":1}}).fetch();
		// 		Meteor.call("getCategoryWiseSWTTSearchFranchise",this.state.categoryName,this.state.subCategory,monthDateStart,monthDateToSess,this.state.studentNameCWTM,(err,res)=>{
		// 			if(err){
		// 				console.log(err);
		// 			}else{
		// 				if(res){
       ...
       return (
		...
		<div>
			...
			<div className="col-lg-12 col-md-12  col-sm-12 col-xs-12 paddingleftzero">
				<div className="col-lg-4 col-lg-offset-8 col-md-12 searchTableBoxAlignSETReport">
		       		<span className="blocking-span">
		           		<input type="text" name="search"  className="col-lg-12 col-sm-12 SearchExam SearchStudentCWTMName inputTextSearch" onInput={this.getSWTMTextValue.bind(this)} required/>
		...
		:
			<div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 selectCatSubCatListt paddingleftzero">
				...
				<div className="col-lg-6 col-md-6 searchbox paddingleftzero">
	       		<span className="blocking-span">
	           		<input type="text" name="search"  className="col-lg-4 col-sm-4 SearchExam SearchStudentCWTMName inputTextSearch" onInput={this.getSWTMTextValue.bind(this)} required/>
	           		<span className="floating-label">Search Student Name</span>
	       		</span>
	        </div>
			</div>
